...
function Public.deep_equals(table1, table2) --Checks if two objects are identical. ie returns true if {"space-science-pack",1} and {"space-science-pack",1} are compared from different object references
...
function Public.remove(list, objectToRemove) --Removes object from list
    if list then
...
function Public.contains(list,object) --Check if object exists in list.
...
function Public.soft_insert(list,objectToAdd) --Adds object to list if it doesn't already exist. 
...
function Public.ammend(old, new)
    old = Public.merge(old, new)
end
